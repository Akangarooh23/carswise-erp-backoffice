/**
 * Decir dentro del correo lo que va adjunto.
 *
 * Un adjunto que el cuerpo no menciona es un adjunto que no se abre. El que lo
 * recibe lee «necesitamos la factura», contesta, y el PDF que iba con el correo
 * se queda ahí — sobre todo cuando el que lee está en otro idioma y en otra
 * empresa, que es exactamente nuestro caso.
 *
 * Y funciona en las dos direcciones: si el cuerpo dice que va la factura de
 * compra y no va, quien revisa lo ve antes de mandarlo. Hoy no hay forma de
 * darse cuenta hasta que el de fuera contesta preguntando por ella.
 *
 * **Se nombran por lo que son, no por cómo se llama el fichero.**
 * «rechnung_final_v2.pdf» no le dice nada a nadie; «Kaufrechnung» sí. El nombre
 * del fichero va detrás entre paréntesis, porque es lo que verá en su cliente de
 * correo y tiene que poder emparejarlos.
 *
 * Al alemán se le escribe en alemán. Estos correos ya salen en dos idiomas y
 * una línea suelta en español dentro de un correo alemán es justo la que se
 * salta.
 */
