...
// ── Generate portal commission invoices from existing external sales ───────────
// (for leads marked Vendido from external portals that don't have a provider invoice yet)
providerBillingRouter.post('/provider-billing/generate-commissions', requireRole(['admin', 'operations']), async (_req, res) => {
  try {
    const leads = await query(`
      SELECT l.id, l.contact_name, l.user_email, l.vehicle_title, l.portal,
             COALESCE(l.sale_price, mo.price) AS sale_price,
             COALESCE(l.created_at) AS date
      ...
      WHERE l.status = 'Vendido'
        ...
        AND COALESCE(l.sale_price, mo.price) IS NOT NULL
        AND l.id NOT IN (SELECT contract_id FROM moveadvisor_provider_invoices WHERE type = 'portal_commission')
    `);

    const created = [];
    for (const lead of leads.rows as Record<string, string>[]) {
      const salePrice = Number(lead.sale_price);
      const commission = Math.round(salePrice * 0.01 * 100) / 100;
      ...
      await query(
        ...
        [id, providerName, lead.id, lead.vehicle_title, lead.contact_name, lead.user_email, salePrice, commission]
      );
      created.push(id);
    }
    res.json({ ok: true, created: created.length, ids: created });
  } catch (err) {
    res.status(500).json({ ok: false, error: 'generate_failed', detail: (err as Error).message });
  }
});
