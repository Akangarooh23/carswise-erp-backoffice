...
const PARTNER_COLORS: Record<string, string> = {
  norauto: 'bg-orange-100 text-orange-700',
  midas: 'bg-yellow-100 text-yellow-700',
  carglass: 'bg-blue-100 text-blue-700',
  euromaster: 'bg-green-100 text-green-700',
  kwik_fit: 'bg-purple-100 text-purple-700',
...
interface EditForm {
  ...
  hoursFreeMode: boolean;
  hoursFreeText: string;
}
...
function workshopToForm(w: WorkshopLocation): EditForm {
  ...
  return {
    ...
    lon:          w.lon != null ? String(w.lon) : '',
    phone:        w.phone ?? '',
    website:      w.website ?? '',
    partner:      w.partner ?? 'independent',
...
export default function WorkshopsPage() {
  ...
  const load = useCallback(async (p = page) => {
    ...
    setLoading(false);
  }, [q, filterProvince, filterPartner, filterActive, page]);

  useEffect(() => { setPage(1); }, [q, filterProvince, filterPartner, filterActive]);
  useEffect(() => { load(page); }, [page, load]);

  function openEdit(w: WorkshopLocation) {
    setSelected(w);
  ...
  async function save() {
    ...
    if (res.ok) {
      ...
      setSaveError('Error al guardar. Comprueba los datos.');
    }
  }
  ...
  return (
    <div>
      <PageHeader
        title="Talleres"
        subtitle={`${total.toLocaleString('es-ES')} talleres en el directorio`}
      />

      {/* Filters */}
      <div className="flex flex-wrap gap-3 mb-5">
        <SearchInput value={q} onChange={setQ} placeholder="Nombre, ciudad, dirección…" className="w-72" />

        <input
          value={filterProvince}
          onChange={(e) => setFilterProvince(e.target.value)}
          placeholder="Provincia…"
          className="px-3 py-2 text-sm border border-slate-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 w-40"
        />
        ...
        >
          ...
          {PARTNER_OPTIONS.map((p) => (
            <option key={p} value={p}>{PARTNER_LABELS[p] ?? p}</option>
          ))}
        </select>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12 text-slate-400 text-sm">Sin resultados. Ajusta los filtros.</div>
        ) : (
          <>
                ...
                <tbody>
                  {items.map((w) => (
                    <tr key={w.id}>
                      ...
                      <td className="text-xs text-slate-400 max-w-[140px] truncate" title={w.business_hours ?? ''}>
                        {w.business_hours ? '✓' : '–'}
                      </td>
                      <td>
          ...
          <div className="space-y-5">
            ...
            <div className="border border-slate-200 rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  type="button"
                  onClick={() => setForm({ ...form, hoursFreeMode: !form.hoursFreeMode })}
              ...
              {form.hoursFreeMode ? (
                ...
                <div className="space-y-3">
                  {([
                    ...
                    ['sab', 'Sábado'],
                    ['dom', 'Domingo'],
                  ] as [keyof StructuredHours, string][]).map(([day, label]) => (
                        ...
                        <>
                          ...
                          {day !== 'dom' && (
                            <div className="flex items-center gap-1.5">
                              ...
                              <input
                                value={form.hours[day].afternoon}
