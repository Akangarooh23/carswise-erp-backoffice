...
export default function BillingPage() {
  ...
  useEffect(() => {
    ...
    if (tab === 'subscribers' || tab === 'trials') {
      ...
      api.get<unknown[]>(`/contracts?type=${type}&page=${page}&limit=50`).then((r) => {
        ...
        if (tab === 'ventas') {
          setSales(r.data as SaleInvoice[]);
        } else {
          setRentings(r.data as RentingInvoice[]);
        }
      }).finally(() => setLoading(false));
    }
  }, [tab, page, planFilter]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Facturación clientes" subtitle="Suscripciones, ventas de vehículos y contratos de renting" />
                      ...
                      {rentings.map((r) => (
                        <tr key={r.id}>
                          ...
                          <td className="text-sm font-semibold text-slate-700 whitespace-nowrap">
                            {r.monthly_price && r.duration_months
                              ? fmtEur(r.monthly_price * r.duration_months)
                              : '–'}
                          </td>
                          <td>
